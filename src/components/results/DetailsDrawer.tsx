import { useEffect, useState, type JSX } from 'react';

import useUrlParams from '@/hooks/useUrlParams';

import CardDetails from './CardDetails';
import Spinner from '../shared/ui/Spinner';
import { useGetAnimeByIdQuery, useRefreshAnimeByIdMutation } from '@/services/apiSlice';
import { extractErrorMessage } from '@/utils/utils';

const DetailsDrawer = (props: { id: number }): JSX.Element => {
  const [visible, setVisible] = useState(false);
  const { deleteParam } = useUrlParams();
  const { id } = props;
  const { data, error, isLoading, isFetching } = useGetAnimeByIdQuery(id, { skip: !id });

  useEffect(() => {
    if (!id) return;

    const timeout = setTimeout(() => setVisible(true), 300);

    return () => {
      document.body.classList.remove('overflow-hidden');
      setVisible(false);
      clearTimeout(timeout);
    };
  }, [id]);

  const handleClose = () => {
    setVisible(false);
    setTimeout(() => {
      deleteParam('details', { replace: true });
    }, 300);
  };

  const [refreshAnimeById, { isLoading: isRefreshing }] = useRefreshAnimeByIdMutation();

  const errorMessage = extractErrorMessage(error);

  return (
    <>
      <div
        data-testid="overlay"
        onClick={handleClose}
        className={`${id ? 'fixed' : 'hidden'} top-0 left-0 w-full h-full bg-black opacity-70 z-40`}
      ></div>
      <section
        className={`flex flex-col gap-4 fixed top-0 h-full lg:w-1/2 sm:w-2/3 right-0 bg-white dark:bg-neutral-800 shadow-lg p-4 z-50 transform transition-transform duration-300 overflow-y-scroll ${visible ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {isLoading && (
          <div className="m-auto">
            <Spinner />
          </div>
        )}
        {isFetching && !isLoading && <p className="text-lg m-auto">Reloading...</p>}
        {error && <p>{errorMessage}</p>}
        {!isLoading && !isFetching && !error && data?.data && (
          <CardDetails
            card={data.data}
            onClose={handleClose}
            onRefresh={() => refreshAnimeById(id)}
            isRefreshing={isRefreshing}
          />
        )}
      </section>
    </>
  );
};

export default DetailsDrawer;
